...
if __name__ == "__main__":
    ...
    # train_ds = SubnetDataset("/home/dtpthao/workspace/vocal-folds/data/aim", train_val="train", transform=transform, fold=fold)
    # train_dl = DataLoader(
    #     train_ds, batch_size=1, drop_last=False, collate_fn=collate_fn, shuffle=True,
    # )
    
    val_ds = SubnetDataset("/home/dtpthao/workspace/vocal-folds/data/aim", train_val="val", fold=fold)
    val_dl = DataLoader(
        val_ds, batch_size=1, drop_last=False, collate_fn=collate_fn, shuffle=True
    )
    

    # # model = REAL(roip_output_size=(8, 8), dim=896)
    # model = MEAL(dim=896)
