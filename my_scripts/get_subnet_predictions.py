...
# target_layers = [subnet.conv11_global_pooling]
...
for file_name in tqdm(os.listdir(root_path)):
    ...
    #         new_preds.append(torch.tensor(new_pred).to('cpu'))
    #     preds = new_preds
    with open("/home/dtpthao/workspace/predictions/yolov5/{}.txt".format(file_name), 'w') as f:
        for x1, y1, x2, y2, conf, cls in preds[0].numpy().tolist():
            f.write("{:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {}\n".format(x1, y1, x2, y2, conf, int(cls)))
    # break
    # ----------------------------------------------------------------------------------------------------------#
